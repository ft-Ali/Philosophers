...
/*   Updated: 2024/10/18 11:58:21 by alsiavos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	write_status(t_estatus status, t_philo *philo, bool debug)
{
	...
	else
	{
		...
		if ((status == FORK_RIGHT || status == FORK_LEFT)
			...
			printf(GREY "%ld ms" RESET " %d has taken a fork\n", elapsed,
				philo->id);
		else if (status == EAT && !simulation_end(philo->data))
			printf(GREEN "%ld ms" RESET " %d is eating\n", elapsed, philo->id);
		else if (status == SLEEP && !simulation_end(philo->data))
			printf(CYAN "%ld ms" RESET " %d is sleeping\n", elapsed, philo->id);
		else if (status == THINK && !simulation_end(philo->data))
			printf(YELLOW "%ld ms" RESET " %d is thinking\n", elapsed,
				philo->id);
		else if (status == DIED && simulation_end(philo->data))
			printf(RED "💀 %d died in %ld ms 💀\n" RESET, philo->id, elapsed - 1);
